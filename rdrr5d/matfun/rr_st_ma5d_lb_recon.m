function [ D1 ] =rr_st_ma5d_lb_recon(D,Data5Dmask,flow,fhigh,dt,N,Niter,eps,verb,mode,a,K,e,ws)
%   RR +  ST + MA: Rank-reduction (RR) + soft thresholding (ST) + moving-average (MA) operators for simultaneous denoising and 
...
Data5Dmask=squeeze(Data5Dmask(1,:,:,:,:));
...
%         end
        ...
        Sn=a(iter)*S_obs+(1-a(iter))*Data5Dmask.*Sn+(1-Data5Dmask).*Sn;
...
function [dout]=P_R(din,N,K,e,ws)
...
%           [U,B,V]=svds(din,N);
             [U,B,V]=svd(din);
           dout=STMA(U(:,1:N)*B(1:N,1:N)*(V(:,1:N)'),K,e,ws);
%         end
%     case 1 %LB
...
%         dout=optshrinkcyk_damp(din,N,NN);
%     case 3 % use optshrink
        
%         dout=optshrinkcyk(din,N);
%     otherwise
