function [ D1 ] = denoising_recon_ma_drr(D,MASK,flow,fhigh,dt,N,NN,Niter,eps,verb,mode,iflb,a,e,ws)
%  RR + damping + MA: Damped rank-reduction (DRR) method + moving average (MA) filter for simultaneous denoising and 
...
function [dout]=P_R(din,N,iflb,NN,e,ws)
...
%         [U,B,V]=svds(din,N+1); % a little bit slower for small matrix
       ...
       dout=MA(U(:,1:N)*B(1:N,1:N)*(V(:,1:N)'),e,ws);  
