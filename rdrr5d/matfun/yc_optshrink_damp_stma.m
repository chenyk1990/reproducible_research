function [Shat,relmse_hat,mse_hat] = yc_optshrink_damp_stma(Xtil ,r ,K ,u ,e ,ws)
% Damped optshrink combined with the soft thresholding moving-average (STMA) operator by Oboue et al., 2020
% 
%          K = damping factor
% 
...
Xtil=STMA(Xtil,u,e,ws); % Apply the STMA operator to the input signal (Xtil) before the singular value decomposition (SVD).

[Util,Stil,Vtil] = svd(Xtil); stil = diag(Stil);

stil=STMA(stil,u,e,ws); % Apply the STMA operator to the singular value (stil) after SVD.
