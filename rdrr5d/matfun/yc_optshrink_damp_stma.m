function [dout] = yc_optshrink_damp_stma(din,N,K, u ,e ,ws)
%yc_optshrink_damp_stma: Optimally shrink the singular values with damping and stma
% BY Yangkang Chen, Sep, 2017
% Modified by Min Bai, May, 2018
% Modified by Oboue et al., February, 2021
% 
% INPUT
% din: input
%   N: rank
...
%
% REFERENCE
% Obue et al., 2020, Robust damped rank-reduction method for simultaneous denoising and reconstruction of 5-D seismic data. Geophysics, 86(1):1–143.
...
din=STMA(din,u,e,ws); % Apply the STMA operator to the input signal (din) before the singular value decomposition (SVD).

[U,S,V]=svd(din);

S=STMA(S,u,e,ws); % Apply the STMA operator to the singular value (S) after SVD.
