function [ D1 ] = drr_st5d_lb_recon(D,MASK,flow,fhigh,dt,N,NN,Niter,eps,verb,mode,iflb,a,K)
%  RR + damping + ST: Damped rank-reduction (DRR) method + soft thresholding (ST) operator for simultaneous denoising and 
%        reconstruction of 5D seismic data with Lanczos bidiagonalization
%
%  IN   D:   	intput 5D data
...
function [dout]=P_R(din,N,iflb,NN,K)
...
%         dout=U(:,1:N)*B(1:N,1:N)*(V(:,1:N)');
        ...
        dout=ST(U(:,1:N)*B(1:N,1:N)*(V(:,1:N)'),K);  
