...
class Autoencoder(object):
    ...
    def __init__(self, n_layers, transfer_function=tf.nn.softplus, optimizer=tf.compat.v1.train.AdamOptimizer(), ae_para=[0, 0]):
        ...
        self.sparsity_level = 0.05  # np.repeat([0.05], self.n_hidden).astype(np.float32)
    ...
    def kl_divergence(self, p, p_hat):
        return tf.reduce_mean(p * tf.compat.v1.log(tf.clip_by_value(p, 1e-8, tf.reduce_max(p)))
                              - p * tf.compat.v1.log(tf.clip_by_value(p_hat, 1e-8, tf.reduce_max(p_hat)))
                              + (1 - p) * tf.compat.v1.log(tf.clip_by_value(1-p, 1e-8, tf.reduce_max(1-p)))
                              - (1 - p) * tf.compat.v1.log(tf.clip_by_value(1-p_hat, 1e-8, tf.reduce_max(1-p_hat))))
