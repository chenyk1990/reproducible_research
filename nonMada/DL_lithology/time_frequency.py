...
def spectrum_cwt(t, data, wavelet='mexh', widths=200, cmap='RdBu', 
    ...
    #maximum = colorscale*maximum
    np.clip(cwtmatr, -maximum, maximum, cwtmatr)
    levels = np.linspace(-maximum, maximum, contour_dvision)
    if plot==True:
        plt.figure(figsize=figsize)
        plt.contourf(t, freqs, cwtmatr, levels=levels, cmap=cmap)
...
def spectrum_cwt2(t, data, wavelet='mexh', widths=200, cmap='RdBu', 
    ...
    if plot==True:
        plt.figure(figsize=figsize)
        plt.contourf(freqs, t, cwtmatr.T, levels=levels, cmap=cmap)
        #plt.ylim(min(freqs), freqmax) #
        plt.xlim(10, 80)
