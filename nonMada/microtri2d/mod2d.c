/* 2D elastic wavefield modeling using the pseudo-spectral method  */
/*
...
int afd(float **wvfld, float **dat, float **dat_v, float *img, float *vel, pspar par, bool mig)
/*< pseudo-spectral wave extrapolation >*/
{
    /*survey parameters*/
    int   nx, nz;
...
int efd(float **wvfld, float **dat, float **dat_v, float *img, float *vel, pspar par, bool mig)
/*< pseudo-spectral wave extrapolation >*/
{
    /*survey parameters*/
    int   nx, nz;
...
int main(int argc, char* argv[])
{
    ...
    if (!sf_getbool("fd",&fd)) fd=false; if(fd) ps=false; /* use fourth-order finite difference */
    ...
    /*do the work*/
    if(fd)
	afd(wvfld, dat, dat_v, img, vel, par, mig);
	else
