function [d,coef] = yc_ani_thresh(c, sorh, thr, thrtype)
%YC_ANI_THRESH: A modified version of ani_thresh
%
%	 [ D1 ] = yc_ani_thresh(c, sorh, thr, thrtype)
%
...
if thrtype=='g'
   ...
   d{1} = c{1};
    for k = 2:m*n
        d{k} = yc_wthresh(c{k}, sorh, lambda);
    end
else     
   d{1} = c{1};
    for k = 2:m*n
