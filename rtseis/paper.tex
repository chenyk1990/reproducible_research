...
\begin{abstract}
    ...
    representation on test datasets. %\par
\end{abstract}
...
\section{Introduction}
    ...
    \cite{Fomel2010a} introduced the seislet transform, which is a digital 
    ...
    The \old{Relative Time}\new{relative time} (RT) \old{Volume}\new{volume}, $\tau(x,y,t)$, has different meanings in
    different domains. 
    In seismic image domain, the \old{Relative Time}\new{RT} is the same as the 
    \old{Relative Geologic Time}\new{relative geologic time}~\cite[]{Stark2003, Stark2004} and each RT contour 
    corresponds to a geologic horizon.
    However, RT volumes of seismic gathers \old{don't}\new{do not} necessarily have geologic 
    ...
    The RT volume already has \old{successful applications}\new{been successfully applied} in missing well-log 
    data prediction \cite[]{Bader2018}\new{ and} seismic horizons construction 
    \cite[]{Wu2015}\old{ and sedimentological interpretation}.
    
    In this paper, we propose a new implementation of the seislet transform, 
    called RT-seislet transform, by using the \old{Relative Time}\new{RT} 
    ...
    transform and the estimation of \new{a} RT volume. 
...
\subsection{Reviews of 2D Seislet Transform}
    ...
    \begin{enumerate}
        \item Arrange the input data as a sequence of records. 
            In the seislet transform, the input data \old{is}\new{are} common shot gather or 
            seismic image.
        \item Divide the arranged records into even and odd components 
            $\mathbf{e}$ and $\mathbf{o}$. For the 2D seislet transform, each 
            trace is a component and the seismic data \old{is}\new{are} split into two parts 
    ...
    Following the inverse lifting scheme from coarse \old{scale }to fine scale, 
    the inverse seislet transform can be constructed.

\subsection{Relative Time volume estimation}
    In the proposed method, \old{the }predictive painting~\cite[]{Fomel2010b}, \old{which
    is }based on the prediction operator arising from PWD~\cite[]{Fomel2002}, is 
    utilized to generate the \old{Relative Time}\new{RT} volume\old{,}\new{.} 
    ...
    trace, is \old{simply }defined as
    \begin{equation}
        \label{predict}
    ...
    using \new{e}\old{E}quation \ref{pp}. 
    If the information spread by the predictive painting \old{is}\new{are} the time \old{value}\new{coordinates}, an 
    ...
    Figure~\ref{fig:dip-pad}, which \old{is}\new{are} estimated by PWD. 
    Figure~\ref{fig:pick-1} is the \old{Relative Time}\new{RT} volume calculated 
    ...
    information it contains, since \old{Relative Time}\new{RT} indicates how 
    ...
    calculating the average of all estimated \old{Relative Time}\new{RT} 
    volumes.
    ...
    trace \old{doesn't}\new{does not} necessarily have information about all seismic events since 
    ...
    \multiplot{2}{pick-1,pick-5}{width=0.6\columnwidth}{\old{Relative time}
        \new{RT} estimated by predictive painting from Figure~\ref{fig:sigmoid} 
        using (a) a single reference trace and (b) multiple reference traces.}
    \multiplot{2}{flat-1,flat-5}{width=0.6\columnwidth}{Flattened images using
        RT volumes from (a) Figure \ref{fig:pick-1} and (b) 
        Figure~\ref{fig:pick-5}.}

\subsection{RT formulation of prediction and update operators}
    \old{Relative time}\new{RT} contains the relationship between traces. 
    ...
    According to \new{e}\old{E}quation \ref{new_form}, the prediction of the 
    ...
    In the proposed method, we use \new{e}\old{E}quation \ref{new_form} to 
    define operators $\mathbf{S}$ in \new{e}\old{E}quations \ref{P} and 
    ...
    \old{Relative Time}\new{RT} attribute instead of the recursive computation 
    ...
    predictions of traces around discontinuities using \new{e}\old{E}quation 
    \ref{new_form} are accurate. 
    Therefore, a better \old{approximation}\new{delineation} of faults and unconformities can be 
    achieved.

    In computing an RT volume, we need to first choose one or multiple reference 
    traces. 
    After computing the RT volume, the \old{Relative Time}\new{RT} attribute 
    between any two traces is easily obtained from one RT volume using\old{ the 
    ...
    $\tau_{r,l}(t)$. 
    According to \new{e}\old{E}quation \ref{rt}, given the shift information 
    ...
    Then, \new{e}\old{E}quation \ref{new_form} is applied to implement the 
    ...
    \old{how to utilize}\new{implementing} \new{e}\old{E}quation~\ref{rt} to compute an RT volume from 
    another \old{RT volume}\new{one} with different reference traces. 
    ...
    contain similar information, which \old{proves}\new{demonstrate} that 
    \new{e}\old{E}quation~\ref{rt} can be used to get the relationship between 
    ...
    \multiplot{4}{pick-150,invint,pick-50,pick-50-true}{width=0.4\columnwidth}{
        ...
        trace is the 150th trace. (b) Time warping of (a). (c) RT 
        volume obtained by \new{e}\old{E}quation~\ref{rt}. (d) The RT volume 
        obtained by the predictive painting and the reference trace is the 50th 
        trace.}
...
\subsection{Workflow}
    ...
    of the input seismic data such that the \old{Relative Time}\new{RT} volume 
    can be computed by the predictive painting from the estimated slopes, as 
    discussed in the previous section. 
    \old{Then with}\new{With} the help of the generated RT volume, the RT-seislet transform is 
    \new{then} applied to the input data to get coefficients in the RT-seislet transform 
    domain. 
...
\section{Synthetic Data Example}
    ...
    Figure~\ref{fig:rt} shows the \old{Relative Time}\new{RT} calculated from 
    ...
    \inputdir{sigmoid}
    \plot{rt}{width=0.8\columnwidth}{\old{Relative time}\new{RT} volume 
        estimated by the predictive painting using multiple reference traces.}
    \multiplot{2}{pwdseisrec1,rtseisrec1}{width=0.6\columnwidth}{Synthetic 
        seismic image reconstruction by thresholding coefficients and keeping 
        only 1\% of significant coefficients using (a) inverse PWD-seislet 
        transform and (b) inverse RT-seislet transform.}
    \plot{coef}{width=0.8\columnwidth}{Normalized coefficients from the 
        PWD-seislet transform (red \new{dashed} line) and the RT-seislet 
        transform (blue \new{solid} line) sorted from large to small on a 
        decibel scale.}
    ...
    \plot{tim}{width=0.80\columnwidth}{CPU time of the PWD-seislet transform 
        (red \new{dashed} line) and the RT-seislet transform (blue \new{solid}
        line) \old{vs.}\new{versus} the number of traces.}
    ...
    superior performance \old{with less sensitivity}\new{that is less sensitive} to noise.
    ...
    \plot{snr}{width=0.80\columnwidth}{SNR diagram of the PWD-seislet transform 
        (red \new{dashed} line) and the RT-seislet transform (blue \new{solid}
        line) with respect to the noise level (variance value).}
    ...
    Figure~\ref{fig:para,para-rtseisrec1,para-diff} presents another synthetic example. 
    ...
    We use \old{only the middle trace}\new{the first, the middle and the last 
    traces} as the reference trace\new{s} to compute the 
    \old{Relative Time}\new{RT} volume.
    \new{The residual between the synthetic shot gather and the reconstruction
    ...
    \inputdir{synth}
    \multiplot{3}{para,para-rtseisrec1,para-diff}{width=0.45\columnwidth}{
        (a) Synthetic shot gather. (b) Data reconstruction by the inverse 
        RT-seislet transform using only 1\% of the most significant 
        coefficients. (c) Difference between (a) and (b).}
...
\section{2-D Field Data Example}
    ...
    Figure~\ref{fig:seisrec1}. The reconstructed image \old{contains}\new{retains} the most 
    ...
    interpolate the \old{Relative Time}\new{RT} volume by a factor of four. 
    ...
    \multiplot{2}{seis,seisrec1}{width=0.60\columnwidth}{(a) The field data in 
        the RT-seislet transform domain and (b) \old{the field data }reconstruction using only 1\% of significant coefficients by the inverse 
        RT-seislet transform.}
    \multiplot{4}{gath,seisdeno,seis16,diff}{width=0.45\columnwidth}{\new{(a) Original gather. }(b) \old{Denoising}\new{Denoised} 
        result by muting RT-seislet coefficients at fine scales (a) and (c) 
        the removed noise section using the RT-seislet transform.}
    \multiplot{2}{seisrand2,gath4}{width=0.6\columnwidth}{(a) Extended seislet 
        ...
        transform.}
    \multiplot{2}{fft,fft2}{width=0.6\columnwidth}{F-K \old{spectrum}\new{spectra} of (a) 
        zero-padded \old{field data} and (b) the interpolated field data.}

\section{3-D Field Data Example}
    ...
    estimated, the \old{Relative Time}\new{RT} volume (Figure~\ref{fig:pick}) is 
    ...
    The proposed method has \old{excellent}\new{good} performance for reconstruction and 
    preserving the structures of the dataset.

    \inputdir{teapot}
    \multiplot{2}{cuber,pick}{width=0.6\columnwidth}{(a) Teapot dome dataset. 
        (b) \old{Relative time}\new{RT} volume estimated by the predictive 
        painting.}
...
\section{Discussion}
    ...
    can be obtained from the \old{Relative Time}\new{RT} volume, which helps 
    ...
    More reference traces usually mean a better \old{Relative Time}\new{RT} 
    volume. 
...
\section{Conclusions}
    ...
    formulation of the seislet transform in reconstructing the \old{original 
    seismic dataset}\new{raw seismic data}. 
    It also has excellent performance in obtaining the sparse representation of
    seismic data.
    Synthetic \old{data }and field data examples show that the proposed new 
