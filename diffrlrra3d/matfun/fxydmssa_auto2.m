function [ D1,H ] = fxydmssa_auto(D,flow,fhigh,dt,Nmin,Nmax,NN,verb,mode,f,eps)
...
for k=ilow:ihigh
    ...
    if k~=ikf
    M=P_R(M,Nmin,Nmax,NN,mode,eps);
    else
    [M,H]=P_R(M,Nmin,Nmax,NN,mode,eps);
    end
...
function [dout,H]=P_R(din,Nmin,Nmax,NN,mode,eps)
