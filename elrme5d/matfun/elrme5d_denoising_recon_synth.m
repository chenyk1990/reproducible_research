function [ D1 ] = elrme5d_denoising_recon_synth(D,MASK,flow,fhigh,dt,N,NN,Niter,eps,verb,mode,a,ws,e1,o1,b1,e0,o0,b0)
%  ELRME: Enhanced low-rank matrix estimation (ELRME) for simultaneous denoising and reconstruction of five-dimensional seismic data
%
%  IN   D:   	intput 5D data
...
for k=ilow:ihigh
    ...
    for iter=1:Niter
        ...
        M=P_R(M,N,NN, ws, e1, o1, b1, e0, o0, b0);
...
function [dout]=P_R(din,N,NN, ws, e1, o1, b1, e0, o0, b0)
% Rank reduction on the block Hankel matrix
[n1,n2]=size(din);

X = atanTma1(din, ws, e1, o1, b1);

        [T,B,V]=svd(X); % a little bit slower for small matrix
        
        for j=1:N
            B(j,j)=B(j,j)*(1-B(N+1,N+1)^NN/B(j,j)^NN);
        end

        B=diag(atanTma0(diag(B), ws, e0, o0, b0));
        
        dout= T(:,1:N)*B(1:N,1:N)*(V(:,1:N)');
